...
// File contracts/dex/interfaces/IUniswapRouter01.sol
...
// File contracts/dex/interfaces/IUniswapRouter02.sol
...
// File contracts/dex/libraries/SwapLibrary.sol
            ...
            uint(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"b79803c7b3e6448ffb477bfb01feef39c5f9e33a23c5e1c3c444426561221cf5" // init code hash
                    )
                )
            )
        );
...
// File contracts/dex/AbstraRouter.sol

// SPDX-License-Identifier: MIT

pragma solidity =0.6.6;

contract AbstraRouter is IUniswapRouter02 {
    using SafeMath for uint;
    ...
    function _swap(
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2
                ...
                : _to;
            IUniswapPair(SwapLibrary.pairFor(factory, input, output)).swap(
                amount0Out,
                amount1Out,
                to,
                new bytes(0)
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0, ) = SwapLibrary.sortTokens(input, output);
            IUniswapPair pair = IUniswapPair(
